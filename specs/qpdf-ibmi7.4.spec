...
#
# 1. Do NOT use the %%cmake macro: on IBM i 7.4, /bin/sh is QShell (QZSHSH) which
#    cannot parse ${VAR:--flags} syntax that %%cmake injects for CFLAGS defaults,
#    and cc (/QOpenSys/pkgs/bin/cc) is gcc-6 which tries to exec 'as' (absent
#    in PASE). Call cmake and ninja directly instead.
#
# 2. gcc12/g++12 mandatory: QPDF 12.x requires C++20; gcc6 only supports C++17.
#
# 3. -pthread / -D_THREAD_SAFE mandatory: gcc12 std_mutex.h includes atomic_wait.h
...
#
# 4. OpenSSL crypto: GnuTLS is not available on IBM i PASE yum repos.
#
# 5. ar/ranlib on IBM i PASE require -X64 to handle ELF 64-bit objects.
#    rules.ninja is patched post-cmake because CMake hardcodes the ar command.

export PATH=/QOpenSys/pkgs/bin:/QOpenSys/usr/bin:/QOpenSys/usr/ccs/bin:/usr/bin:$PATH
export CC=/QOpenSys/pkgs/bin/gcc-12
export CXX=/QOpenSys/pkgs/bin/g++-12
export PKG_CONFIG_PATH=/QOpenSys/pkgs/lib/pkgconfig

mkdir -p %{_builddir}/%{name}-%{version}/ibm-ibmi7.4-build
cd %{_builddir}/%{name}-%{version}/ibm-ibmi7.4-build

/QOpenSys/pkgs/bin/cmake %{_builddir}/%{name}-%{version} \
    -DCMAKE_INSTALL_PREFIX=/QOpenSys/pkgs \
    -DCMAKE_INSTALL_SBINDIR=bin \
    -DBUILD_SHARED_LIBS=ON \
    -DCMAKE_C_COMPILER=/QOpenSys/pkgs/bin/gcc-12 \
    -DCMAKE_CXX_COMPILER=/QOpenSys/pkgs/bin/g++-12 \
    -DCMAKE_C_FLAGS="-O2 -g -pthread -D_THREAD_SAFE" \
    -DCMAKE_CXX_FLAGS="-O2 -g -pthread -D_THREAD_SAFE" \
    -DCMAKE_EXE_LINKER_FLAGS="-pthread" \
    -DCMAKE_SHARED_LINKER_FLAGS="-pthread" \
    -DBUILD_STATIC_LIBS=OFF \
    -DREQUIRE_CRYPTO_OPENSSL=1 \
    -DUSE_IMPLICIT_CRYPTO=openssl \
    -DOPENSSL_ROOT_DIR=/QOpenSys/pkgs \
    -DOPENSSL_INCLUDE_DIR=/QOpenSys/pkgs/include \
    -DOPENSSL_CRYPTO_LIBRARY=/QOpenSys/pkgs/lib/libcrypto.so \
    -DOPENSSL_SSL_LIBRARY=/QOpenSys/pkgs/lib/libssl.so \
    -DINSTALL_MANUAL=OFF \
    -DSHOW_FAILED_TEST_OUTPUT=1 \
    -GNinja

# Patch rules.ninja: ar/ranlib on IBM i PASE require -X64 for ELF 64-bit objects
sed -i "s!/QOpenSys/usr/bin/ar qc!/QOpenSys/usr/bin/ar -X64 qc!g" CMakeFiles/rules.ninja
sed -i "s!/QOpenSys/usr/bin/ranlib !/QOpenSys/usr/bin/ranlib -X64 !g" CMakeFiles/rules.ninja

/QOpenSys/pkgs/bin/ninja -j4

%install
export PATH=/QOpenSys/pkgs/bin:/QOpenSys/usr/bin:$PATH
cd %{_builddir}/%{name}-%{version}/ibm-ibmi7.4-build
DESTDIR=%{buildroot} /QOpenSys/pkgs/bin/ninja install

# Manual PDF — path relative to source tree, not build dir
# %{_pkgdocdir} is not defined on IBM i PASE RPM macros → use %{_datadir}/doc/%{name}
install -D -m 0644 %{_builddir}/%{name}-%{version}/%{name}-%{version}-doc/%{name}-manual.pdf \
    %{buildroot}%{_datadir}/doc/%{name}/%{name}-manual.pdf

# Bash completion — path relative to source tree
install -D -m 0644 %{_builddir}/%{name}-%{version}/completions/bash/qpdf \
    %{buildroot}%{_datadir}/bash-completion/completions/qpdf

# Zsh completion — path relative to source tree
install -D -m 0644 %{_builddir}/%{name}-%{version}/completions/zsh/_qpdf \
...
- Port to IBM i PASE 7.4 (ppc64/power8)
- Export CC/CXX before %%cmake to prevent gcc-6/missing-as probe on IBM i 7.4
- Add ar/ranlib -X64 patch on rules.ninja for ELF 64-bit objects
- Force OpenSSL paths explicitly (-DOPENSSL_ROOT_DIR etc.) for CMake detection
- Switch to -GNinja generator; -DINSTALL_MANUAL=OFF (no sphinx on PASE)
